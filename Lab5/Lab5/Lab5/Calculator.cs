...
{
    private static SQLiteConnection m_dbConn = default!;
    private static SQLiteCommand m_sqlCmd = default!;
    ...
    public int GetLastMemIndex()//последний порядковый номер вычисленного значения (-1 если нет такого)
    {
        if (lastmem >= 0)
        {
            return lastmem + 1;
    ...
    public bool LoadXML()
    {
        try
        {
            ...
            List<int> MemTemp = (List<int>)serializer.Deserialize(reader);
            reader.Close();
            if (MemTemp!.Count > 0)
            {
                Mem.Clear();
    ...
    public bool LoadJSON()
    {
        try
        {
            ...
            List<int> MemTemp = JsonSerializer.Deserialize<List<int>>(data);
            if (MemTemp!.Count > 0)
            {
                Mem.Clear();
                Mem = MemTemp;
    ...
    public bool CalculateExpression(String S)// S строка введённая пользователем
    {
        if (S.Length == 0)
        {
            return false;
        ...
        bool saveinputnum = inputnum;
        String ST ="";
        for (int i = 0; i < S.Length; i++)
        {
            if ((i > 0 || lastmem>=0) && (S[i] == '+' || S[i] == '-' || S[i] == '/' || S[i] == '*'))//операция
            {
                if (ST.Length > 0)
                {
                    if (!Calculate(ST))
                    {
                        inputnum = saveinputnum;
                        return false;
                    }
                    ST = "";
                }
    ...
    private bool Calculate(String S)// S строка введённая пользователем
    {
        if (S == "+" || S == "-" || S == "/" || S == "*")
        {
            if (!inputnum)
