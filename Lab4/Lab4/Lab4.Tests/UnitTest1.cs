...
namespace Lab4.Tests
{
    public class UnitTest1
    {
        ...
        {            
            ...
            Assert.True(calculator.UpdateMem(3, 13));
            Assert.Equal(new List<int> { 2, 4, 13 }, calculator.GetMem());
            Assert.True(calculator.DeleteMem(3));
            Assert.Equal(new List<int> { 2, 4 }, calculator.GetMem());
            Assert.True(calculator.Calculate("#2"));
